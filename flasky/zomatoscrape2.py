...
import time
from flasky.tar_helper import get_blacklist, get_location_from_placeid, add_keywords
...
def search_city_id(latlongtocenter):
...
def get_cards(html):
    ...
    for div in s:
        ...
        if returnintmeters(distance) < int(radius):
            output.append(link)
    return output
...
def selenium_get(url):
        ...
        driver.quit()
        return get_cards(page_source)

def nearby_places(location, radius, keyword=''):
    params = {
        'radius' : str(radius),
    ...
    if keyword != '':
        params['term'] = keyword
    center = latlongtocenter(location['lat'], location['lng'])
    if center is None:
        return []
    cityurl = search_city_id(center)
    ...
    for item in dineoutlinksearch['pages']['city'][str(cityid)]['sections']['SECTION_QUICK_SEARCH']['items']:
        if item['categoryType'] == 'dineout':
            dineouturl = item['url']
    linklist = selenium_get(dineouturl)
    return linklist
...
def zomplace_to_db(datadict):
    ...
    if zomplace is None:
        ...
        website = url + datadict['pages']['current']['pageUrl'][1:]
        business_status = get_business_status(datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_BASIC_INFO'])
        price_level = get_price_level(datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_RES_DETAILS']['CFT_DETAILS']['cfts'])
...
def place_to_db(datadict, zom_id):
    placerecord = Places.query.filter(Places.zomatoplaceid == zom_id).first()
    if placerecord is None:
        ...
        placerecord = Places(placename=place_name, 
            zomatoplaceid = zom_id, 
            vicinity=vicinity, 
            street1=street1, 
            street2=street2,
            suburb=suburb, 
...
def get_keywords(datadict):
    output = []
    zomatoplace_id=datadict['pages']['current']['resId']
    for cuisine in datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_RES_DETAILS']['CUISINES']['cuisines']:
        output.append(cuisine['name'])
    for highlight in datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_RES_DETAILS']['HIGHLIGHTS']['highlights']:
        output.append(highlight['text'])
    return output

def convert_timing(timingstring):
...
def get_index_of_days(daysstring):
    days = ('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun')
    output = []
    if ', ' in daysstring:
        for mylist in daysstring.split(', '):
            output.extend(get_index_of_days(mylist))
    elif '-' in daysstring:
...
def opening_hours_to_db(datadict, placeid):
    zomatoplace_id=datadict['pages']['current']['resId']
    if len(datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_BASIC_INFO']['timing']['customised_timings']) == 0:
        return
    if 'opening_hours' not in datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_BASIC_INFO']['timing']['customised_timings']:
        return
    timingdict = datadict['pages']['restaurant'][str(zomatoplace_id)]['sections']['SECTION_BASIC_INFO']['timing']['customised_timings']['opening_hours']  
    daysindex = []
    ...
    for hours in timingdict:
        ...
        opening = convert_timing(opening)
        closing = convert_timing(closing)
        daysindex.extend(get_index_of_days(hours['days']))
...
def linklist_to_db(linklist, job_id=0):
    output = []
    for link in linklist:
        myurl = url + link[1:]
        zomplace = ZomatoPlace.query.filter(ZomatoPlace.website == myurl).first()
        if zomplace is None:
            myjson = data_from_url(myurl)
            zomatoid = zomplace_to_db(myjson)
            placeid = place_to_db(myjson, zomatoid)
            zomplace = ZomatoPlace.query.filter(ZomatoPlace.id == zomatoid).first()
            zomplace.placeid = placeid
            add_keywords(get_keywords(myjson), placeid)
            opening_hours_to_db(myjson, placeid)  
        else:
            placeid = zomplace.placeid
        if placeid != 0:
            db_session.add(JobResults(placeid=placeid, jobid=job_id))
            db_session.commit()
        output.append(zomplace.id)
    return output
    

def get_zomato_place_id(placeid):
    myplace = Places.query.filter(Places.id == placeid).first()
    if myplace is None:
        raise Exception(f"Yeah nah.  place {placeid} does not exist." )
    if myplace.zomatoplaceid is not None:
        return myplace.zomatoplaceid
    else:
        mylist = nearby_places(get_location_from_placeid(placeid), 100, myplace.placename)
        mylinklist = linklist_to_db(mylist)
        if len(mylinklist) > 0:
            return mylinklist[0]
        return 0
